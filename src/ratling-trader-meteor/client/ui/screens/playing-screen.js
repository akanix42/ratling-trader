"use strict";
PlayingScreen.typeName = "playingScreen";
PlayingScreen.$inject = ["display", "uiController", "asciiTileFactory", "inventoryScreen", "messageScreen"];
Ui.registerUnique(PlayingScreen.typeName, PlayingScreen);

function PlayingScreen(display, ui, asciiTileFactory, inventoryScreen, messageScreen) {
    this._ = {
        display: display,
        ui: ui,
    ...
    postal.subscribe({
        channel: "display",
        topic: "level.changed",
        callback: function levelChanged(tiles) {
            renderLevel.call(this, tiles);
        }
    });
    postal.subscribe({
        channel: "display",
        topic: "fov.changed",
        callback: function fovChanged(fov)
        {
            renderFov.call(this, fov);
        }
    });
    postal.subscribe({
        channel: "display",
        topic: "tiles.changed",
        callback: function tilesChanged(tiles) {
            renderFov.call(this, tiles);
        }
    });
    postal.publish({
        channel: "display",
        topic: "display.initialized"
    });
...
PlayingScreen.prototype = {
    ...
    handleInput: function handleInput(inputType, inputData) {
        var command = this._.commandMap[inputType][inputData.keyCode];
        if (!command) return;

        if (typeof command === 'function')
            command = command();
        if (command)
            this._.uiToGameBridge.queueInput(command);
        //game.processCommand(command);
    }
};


function renderLevel(tiles) {
    var display = this._.display;
    for (var x = 0; x < display.size.width; x++) {
        ...
        for (var y = 0; y < display.size.height; y++) {
            ...
            if (column === undefined || !column[y]) {
                ...
                uiTile = this._.asciiTileFactory.nullTile;
            }
            else {
                tile = column[y];
                var entities = tile.entities.all();
                uiTile = this._.asciiTileFactory.create(entities[entities.length - 1].type);
            }
...
function renderFov(fov) {
    ...
    //console.log('fov');
    renderCurrentFov.call(this, display, fov.tiles, fov.tilesInFov);
    renderPreviousFov.call(this, display, fov.tiles, fov.tilesInFov);
    this._.previousFov = fov.tilesInFov;
}
    ...
    function renderCurrentFov(display, tiles, fov) {
        ...
        for (var i = 0; i < fovKeys.length; i++) {
            ...
            var uiTile = this._.asciiTileFactory.create(entities[entities.length - 1].type);
    ...
    function renderPreviousFov(display, tiles, fov) {
        var previousFov = this._.previousFov;
        if (previousFov) {
            ...
            for (var i = 0; i < pFovKeys.length; i++) {
                ...
                var uiTile = this._.asciiTileFactory.create(entity.type);
