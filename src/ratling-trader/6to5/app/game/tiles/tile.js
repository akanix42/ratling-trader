...
define(function (require) {
  ...
  function Tile(initialArchitecture, data, entityFactory, eventHub, intentHub) {
    ...
    function getNeighbors(radius) {
      ...
      for (var i = 0; i < radius; i++) neighborsInRadius = neighborsInRadius.concat(getNeighborsAtRadiusLevel(i));

      return neighborsInRadius;
    }

    function getNeighborsAtRadiusLevel(radiusLevel) {
      if (neighbors[radiusLevel] !== undefined) return neighbors[radiusLevel];
    ...
    function updateNeighbors(radiusLevel) {
      ...
      for (var x = left; x <= right; x++) for (var y = top; y <= bottom; x === left || x === right ? y++ : y += (radiusLevel + 1) * 2) if (!(x === getPosition().x && y === getPosition().y)) {
    ...
    function addEntity(entity) {
      if (entity.id in entitiesById) return;
    ...
    function removeEntity(entity) {
      if (!(entity.id in entitiesById)) return;
      for (var i = 0; i < entities.length; i++) if (entities[i] === entity) {
        entities.splice(i, 1);
        break;
      }
