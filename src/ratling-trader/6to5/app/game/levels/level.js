...
define(function (require) {
  ...
  function Level(data, entityFactory, scheduler, logger) {
    ...
    for (var x = 0; x < width; x++) for (var y = 0; y < height; y++) map.tiles[x][y].setMap(self);
    ...
    function addEntity(entity) {
      if (entities[entity.getId()]) return;
    ...
    function getRandomTile(filter) {
      ...
      if (filter !== undefined) filter = extend({}, defaultFilter, filter);
      ...
      function matchesFilter(tile) {
        ...
        for (var i = 0; i < filter.architectures.length; i++) if (filter.architectures[i] === type) return true;
      }
    }
    ...
    function getAdjacentTiles(position) {
      var adjacentTiles = [];
      for (var x = Math.max(position.x - 1, 0); x < Math.min(position.x + 2, width); x++) for (var y = Math.max(position.y - 1, 0); y < Math.min(position.y + 2, height); y++) if (!(x === position.x && y === position.y)) adjacentTiles.push(getTile(x, y));
