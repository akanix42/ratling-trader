...
define(function (require) {
  ...
  function position(entity, initialLevel, initialTile) {
    ...
    function setLevel(newLevel) {
      ...
      level = newLevel;
      if (level) level.addEntity(entity);
    }

    function removeFromCurrentLevel() {
      if (level) level.removeEntity(entity);
    }
    ...
    function getLastKnownPosition() {
      var position = getPosition() || (previousTile ? previousTile.getPosition() : { x: -1, y: -1 });
      return position;
    }
    ...
    function setTile(newTile) {
      ...
      if (!newTile) return;
      previousTile = tile;
      if (previousTile) previousTile.removeEntity(entity);
    ...
    function removeFromCurrentTile() {
      if (tile) tile.removeEntity(entity);
      tile = null;
    }
  }
});