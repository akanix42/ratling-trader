...
define(function (require) {
  ...
  function Entity(data, entityAttributes, entityPositionFactory, loadedMixins, loadedBehaviors, logger, eventHub, intentHub) {
    ...
    function initAI() {
      ...
      function processBehaviors(state) {
        for (var i = 0; i < state.behaviors.length; i++) {
          ...
          if (!("probability" in behavior)) behavior.probability = 1;
    ...
    function addMixins() {
      ...
      for (var i = 0; i < mixins.length; i++) addMixin(mixins[i]);
    }
    ...
    function addMixin(name) {
      ...
      function addEventHandler(fn, handler, registrar) {
        if (!(fn in registrar)) registrar[fn] = [];
    ...
    function act() {
      if (self.perform(actCommand())) return;

      if (!self.hasAlreadyActed) decideOnAction();
    ...
    function decideOnAction() {
      for (var i = 0; i < getState().behaviors.length; i++) {
        ...
        if (behavior.probability >= ROT.RNG.getUniform()) if (self.hasAlreadyActed = performAction(behavior)) return;
      }
    ...
    function invokeHandlers(handlers, data) {
      if (handlers === undefined || handlers.length === 0) return true;
      ...
      for (var i = 0; i < handlers.length; i++) {
        ...
        if (!result.metSuccess && handlerResult) result.metSuccess = true;
        if (!result.metFailure && !handlerResult) result.metFailure = true;
      }
