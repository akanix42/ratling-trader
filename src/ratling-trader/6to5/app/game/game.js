...
define(function (require) {
  ...
  function Game(levelFactory, logger, entityFactory, scheduler) {
    ...
    function processCommand(command) {
      ...
      if (!command || !(command in actions)) return { error: "invalid command" };
    ...
    function moveCursor(command) {
      if (command === GameCommands.GoLeft) cursorPosition.x--;
      if (command === GameCommands.GoRight) cursorPosition.x++;
      if (command === GameCommands.GoUp) cursorPosition.y--;
      if (command === GameCommands.GoDown) cursorPosition.y++;
      ...
      if (isCursorLockedToPlayer) player.getPositionManager().setPosition(cursorPosition.x, cursorPosition.y);
    }
