...
define(function (require) {
  ...
  function attacker(logger, mixinFactory) {
    ...
    function afterAttack(attack) {
      if (attack.wasSuccessful) logger.log(stringFormat("{source.getType} hit {target.getType}", attack));else logger.log(stringFormat("{attack.source.getType} missed {attack.target.getType}"));
    }
  }
});