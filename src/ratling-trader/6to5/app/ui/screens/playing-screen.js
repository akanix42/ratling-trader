...
define(function (require) {
  ...
  function PlayingScreen(screenManager, display, game, asciiTiles) {
    ...
    function render() {
      ...
      if (gameState.isGameOver) screenManager.switchTo("losingScreen");
      ...
      for (var x = cameraPosition.x; x < cameraPosition.x + display.getOptions().width; x++) {
        for (var y = cameraPosition.y; y < cameraPosition.y + display.getOptions().height; y++) {
          ...
          asciiTiles.get(gameTile).draw(display, x - cameraPosition.x, y - cameraPosition.y);
        }
      }
    }
    ...
    function calculateCameraCoordinate(cursorCoordinate, screenSize, mapSize, reference) {
      ...
      if (mapSize < screenSize) return getCameraCoordinate(halfMap - halfScreen, reference);
      if (cursorCoordinate < halfScreen) return getCameraCoordinate(0, reference);
      if (cursorCoordinate > mapSize - halfScreen) return getCameraCoordinate(mapSize - screenSize, reference);

      return getCameraCoordinate(cursorCoordinate - halfScreen, reference);
    }

    function getCameraCoordinate(position, reference) {
      if (reference === undefined) throw "Reference must be defined.";
      if (reference === null) reference = position;
    ...
    function handleInput(inputType, inputData) {
      ...
      if (typeof command === "function") command();else {
