...
define(function (require) {
  ...
  function Injector() {
    ...
    function resolve(name, dependencyAbove, dependencyTree) {
      ...
      if (dependencyAbove && name in dependencyTree) throw dependencyAbove + " has a circular dependency on " + name;
    ...
    var injector = {
      ...
      getDependencies: function (name, dependencies, dependencyTree) {
        return dependencies.length === 1 && dependencies[0] === "" ? [] : dependencies.map(function (value) {
  ...
  function Initializer(injector, name, item, isSingleton) {
    ...
    function createInitializationSteps() {
      if (isFunction(item)) createFunctionInitializationSteps();else createObjectInitializationSteps();
    }
